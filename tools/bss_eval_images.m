...
%
% [SDR,ISR,SIR,SAR]=bss_eval_images(ie,i)
%
% Inputs:
...
SAR=zeros(nsrc,1);
for j=1:nsrc,
    [s_true,e_spat,e_interf,e_artif]=bss_decomp_mtifilt(reshape(ie(j,:,:),nsampl,nchan).',i,j,512);
    [SDR(j),ISR(j),SIR(j),SAR(j)]=bss_image_crit(s_true,e_spat,e_interf,e_artif);
end
